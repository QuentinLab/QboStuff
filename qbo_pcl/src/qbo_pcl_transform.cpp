...
{
public:
	Transformer(std::string target_frame="/map",std::string source_pc_topic="camera/depth/points", std::string target_pc_topic="camera/depth/transformed_points")
	{
...
void Transformer::transformAndPublishPC(const sensor_msgs::PointCloud2 &inputCloud){
	...
	try
	{
		listener.waitForTransform(_target_frame,"/camera_depth_optical_frame",ros::Time(0),ros::Duration(10.0));
	}
